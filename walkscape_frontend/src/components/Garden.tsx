...
export default function Garden() {
    ...
    const loadGardenData = useCallback(async () => {
        ...
        try {
            ...
            const artifactIds = await (contract as unknown as { getPlayerArtifactIds: (address: string) => Promise<string[]> }).getPlayerArtifactIds(address);
            setArtifacts(artifactIds);

            const petIds = await (contract as unknown as { getPlayerPetIds: (address: string) => Promise<string[]> }).getPlayerPetIds(address);
            const petData: Pet[] = [];
    ...
    const handleMintPet = async (petType: PetType) => {
        ...
        try {
            ...
            await (contractWithSigner as unknown as { mintPet: (petType: PetType) => Promise<unknown> }).mintPet(petType);
    ...
    const handleFeedPet = async (petId: string) => {
        ...
        try {
            ...
            await (contractWithSigner as unknown as { feedPet: (petId: string, nutritionScore: number) => Promise<unknown> }).feedPet(petId, 85);
